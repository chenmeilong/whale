...
fcn_model = torch.load('fcn/checkpoints/model100.pkl')
fcn_model.eval()
# 数据增强
data_transforms = {
    'test':
...
def pre_fcn(img_tensor):
    ...
    numpy_tran = np.transpose(img1, (2, 0, 1))  # 图片转置
    tensor_fcn = torch.Tensor(numpy_tran)

    out_tensor =img_tensor.mul(tensor_fcn )
    #print(out_tensor)

    # numpy_tensor = out_tensor.numpy()    #显示输出图片
    # numpy_tran = np.transpose(numpy_tensor, (1, 2, 0))  # 图片转置
    # print(numpy_tran.shape)
    # cv2.imshow('img_1', numpy_tran)
    # cv2.waitKey()
